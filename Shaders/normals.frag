...
vec3 normal = texture(norm_tex, texCoord).rgb; //converts rgb to xyz
normal = normalize(normal * 2.0 -  1.0); //turns 0 into -1 and 1 stays 1
...
//FragColor = vec4((1*(specularDir + diffuseDir + ambientDir)),1.0);
...
//FragColor = vec4(.3f,1.f,1.f,1.0f);
